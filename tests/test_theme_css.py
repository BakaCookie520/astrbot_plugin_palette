...
class ComponentManagementGlassTest(unittest.TestCase):
    def test_management_table_uses_isolated_glass_layer(self) -> None:
        ...
        bodies = _rules_bodies(
            css,
            ".v-card.rounded-lg.overflow-hidden.elevation-1::before",
        )
        self.assertTrue(bodies, "management table glass layer is missing")
        self.assertTrue(
            any(
    ...
    def test_management_and_sidebar_blur_disable_at_zero(self) -> None:
        css = _css({"stats_card_blur": 0})
        table_bodies = _rules_bodies(
            css,
            ".v-card.rounded-lg.overflow-hidden.elevation-1::before",
        )
        ...
        self.assertTrue(
            any("backdrop-filter: none !important;" in body for body in sidebar_bodies)
        )
